...
object MarkdownDocumentParser {
    ...
    fun parse(markdown: String): MarkdownDocument {
        ...
        fun flushParagraph() { if (paragraph.isNotEmpty()) { blocks += MarkdownBlock.Paragraph(paragraph.toList()); paragraph.clear() } }
        fun flushList() { orderedList?.let { blocks += MarkdownBlock.ListBlock(it, items.toList()); items.clear(); orderedList = null } }
        fun addList(isOrdered: Boolean, text: String) { flushParagraph(); if (orderedList != null && orderedList != isOrdered) flushList(); orderedList = isOrdered; items += inline(text) }
        markdown.replace("\r\n", "\n").lines().forEach { line ->
            val headingMatch = heading.matchEntire(line)
            when {
                line.isBlank() -> { flushParagraph(); flushList() }
                headingMatch != null -> { flushParagraph(); flushList(); blocks += MarkdownBlock.Heading(headingMatch.groupValues[1].length, inline(headingMatch.groupValues[2])) }
                ...
                else -> { flushList(); paragraph += inline(line) }
            }
        }
    ...
    private fun inline(text: String): List<MarkdownInline> {
        val result = mutableListOf<MarkdownInline>(); var cursor = 0
        emphasis.findAll(text).forEach { match ->
            ...
            result += if (match.groupValues[1].isNotEmpty()) MarkdownInline.Bold(match.groupValues[1]) else MarkdownInline.Italic(match.groupValues[2])
